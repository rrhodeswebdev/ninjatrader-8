...
from backintime.broker.futures.proxy import FuturesBroker, FuturesBrokerProxy
...
@pytest.fixture
def broker_1601() -> FuturesBrokerProxy:
    """
    Setup FuturesBrokerProxy with initial balance of 1601,
    ...
    return FuturesBrokerProxy(broker)


def test_long_market_buy(broker_1601):
    broker = broker_1601
    ...
    broker._broker.update(test_candle)
    ...
    assert broker.balance.available_usd_balance == Decimal('0.38') # 1601 - init(1600) - fee(0.62)
    assert broker.balance.usd_balance == Decimal('1600.38')  # 1601 - fee(0.62)


def test_long_market_sell(broker_1601):
    broker = broker_1601
    sample_candles = [
    ...
    broker._broker.update(sample_candles[0])
    assert market_buy.status is OrderStatus.EXECUTED
    assert market_buy.trading_fee == Decimal('0.62')
    assert market_buy.fill_price == Decimal('1000')
    assert broker.in_long == True
    ...
    broker._broker.update(sample_candles[1])
    assert market_sell.status is OrderStatus.EXECUTED
    assert market_sell.trading_fee == Decimal('0.62')
    assert market_sell.fill_price == Decimal('1010')
    assert broker.in_long == False
    assert broker.balance.available_usd_balance == Decimal('1619.76')
    assert broker.balance.usd_balance == Decimal('1619.76')


def test_short_market_sell(broker_1601):
    broker = broker_1601
    test_candle = Candle(open_time=datetime.now(),
                         open=Decimal(1000),
                         high=Decimal(1100),
...
def test_short_market_buy(broker_1601):
    broker = broker_1601
    sample_candles = [
        Candle(open_time=datetime.now(),
               open=Decimal(1000),
    ...
    broker._broker.update(sample_candles[1])
    assert short_buy.status is OrderStatus.EXECUTED
    assert short_buy.trading_fee == Decimal('0.62')
    assert short_buy.fill_price == Decimal('1010')
    assert broker.in_short == False
    assert broker.balance.available_usd_balance == Decimal('1579.76')
    assert broker.balance.usd_balance == Decimal('1579.76')


def test_long_tpsl(broker_1601):
    broker = broker_1601
    sample_candles = [
        Candle(open_time=datetime.now(),
               open=Decimal(1000),
    ...
    assert long_buy.take_profit.status is OrderStatus.EXECUTED
    assert long_buy.take_profit.trading_fee == Decimal('0.62')
    assert long_buy.take_profit.fill_price == Decimal('1200')

    assert long_buy.stop_loss.status is OrderStatus.SYS_CANCELLED
    assert long_buy.stop_loss.trading_fee is None
    assert long_buy.stop_loss.fill_price is None

    assert broker.in_long == False
    assert broker.in_short == False
    assert broker.balance.available_usd_balance == Decimal('1999.76')
    assert broker.balance.usd_balance == Decimal('1999.76')


def test_short_tpsl(broker_1601):
    broker = broker_1601
    sample_candles = [
        Candle(open_time=datetime.now(),
               open=Decimal(1000),
    ...
    assert short_sell.take_profit.status is OrderStatus.EXECUTED
    assert short_sell.take_profit.trading_fee == Decimal('0.62')
    assert short_sell.take_profit.fill_price == Decimal('800')

    assert short_sell.stop_loss.status is OrderStatus.SYS_CANCELLED
